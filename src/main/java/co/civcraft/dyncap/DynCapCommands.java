	...
	private boolean getQueueInfo(CommandSender sender, String[] args) {
		...
		if (args.length == 1) {
			...
			if (isInteger(args[0])) {
				...
				if (queueItem != null) {
					return sendMessage(sender, startIndex + "     " + queueItem.getName() + "     " + queueItem
							.getSecondsSinceLastAttempt());
				} else {
			...
			//search by name
				...
				if (index != -1) {
					...
					return sendMessage(sender, index + "     " + queueItem.getName() + "     " + queueItem
							.getSecondsSinceLastAttempt());
				} else {
			...
			if (isInteger(args[0]) && isInteger(args[1])) {
				...
				for (int x = startIndex; x <= endIndex; x++) {
					...
					if (queueItem != null) {
						sendMessage(sender, x + "     " + queueItem.getName() + "     " + queueItem
								.getSecondsSinceLastAttempt());
					} else {
